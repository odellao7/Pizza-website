...
const scrollActive = () => {
    ...
    sections.forEach(current => {
        const sectionHeight = current.offsetHeight, // Fixed the typo here
            sectionTop = current.offsetTop - 58,
            sectionId = current.getAttribute('id'),
            sectionsClass = document.querySelector('.nav__menu a[href*=' + sectionId + ']');

        if (sectionsClass) { // Prevents errors if no matching link is found
...
// Fixed missing dots before class names
